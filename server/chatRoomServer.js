...
socket.on('connection', function(client){
	...
	*/
	...
	client.on('sendInputing', function (data) {
		client.broadcast.emit('inputing', { user: userName, msgType: data.msgType, userList: userList });
		log.debug("client(" + userName + ") [" + clientIp + "] send inputing:\n");
	});
	client.on('stopInputing', function (data) {
		client.broadcast.emit('stopinputing', { user: userName, msgType: data.msgType, userList: userList });
		log.debug("client(" + userName + ") [" + clientIp + "] stop inputing:\n");
	});
	//广播信息给除当前用户之外的用户
