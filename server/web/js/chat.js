var Meeting = {
	...
	showInputing : function(from) {
		this.inputing.html(from+" is inputing......");
	},
	hideInputing : function(from) {
		this.inputing.html("<br/>");
	},
	sendMsg : function() {
	...
	sendInputing : function() {
		if(!this.isInputing)
			socket.emit('sendInputing',{ msgType:this.msgType });
		else
			clearTimeout(this.timeout);
		this.isInputing=true;
	},
	stopInputing : function() {
		var _this = this;
		this.timeout = setTimeout(function(){
		_this.isInputing=false;
		socket.emit('stopInputing',{ msgType:this.msgType });
		},3000);
	},
	getMyMsg : function() {
		if (this.msgType=="txt") {
